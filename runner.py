...
pop = subprocess.Popen(
    [env.PYRUNNER, "App.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
)
...
while pop.poll() is None:
    ...
    output = pop.stdout.readline()  # type: ignore
    if output:
        print(output.strip())
    error = pop.stderr.readline()  # type: ignore
    if error:
