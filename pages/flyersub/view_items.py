...
def show():
    uname = utils.database.getTable("Users").iloc[utils.app.storage.user["logIn"]]["username"]
                ...
                with ui.card().classes("box"):
                    ...
                    with ui.row().classes("justify-end"):
                        ui.button(icon='message').props("fab mini").on_click(
                            lambda _, requester=item['requester']: ui.navigate.to(f"/msg/{requester}")
                        )
                        ui.button(icon="check").props("fab mini").on_click(
                            lambda _, item_id=item['id']: ui.notify(f"Offer to deliver item {item_id}")
                        )
