...
from utils import styles, section, header, username, password, phones, Form, database, dateCheck, unique
import uuid
...
def phone():
    ...
    with ui.grid(columns=2):
        ...
        with ui.element('div').classes('p-2 bg-orange-100'):
            formlabel('Password: ')
        with ui.element('div').classes('p-2 bg-blue-100'):
            pword = ui.input(password_toggle_button=True, value=password.generate_password(f'{uuid.uuid4()}', 14)).props('rounded outlined dense')


        with ui.element('div').classes('p-2 bg-orange-100'):
            formlabel('Confirm Password: ')
        with ui.element('div').classes('p-2 bg-blue-100'):
            pword_cfm = ui.input(password=True, password_toggle_button=True).props('rounded outlined dense')
        ...
        with ui.element('div').classes('p-2 bg-blue-100'):
            with ui.input('Date') as date:
                with ui.menu().props('no-parent-event') as menu:
                    with ui.date().bind_value(date).on_value_change(lambda e: formValidCheck(e)):
