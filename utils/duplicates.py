...
def rank_similarities(directory):
    ...
    for i in range(len(python_files)):
        for j in range(i + 1, len(python_files)):
            with open(python_files[i], encoding='utf8') as file_1, open(python_files[j], encoding='utf8') as file_2:
                ...
                similarities.append((python_files[i], python_files[j], similarity_ratio))
