...
def create_location_input(
    record={}, address_autocomplete=[], formValidCheck=null, inner_prompt=""
):
...
def setup_validation(
    form, fields, date=None, date_options=None, custom_validation=None
):
    ...
    def formValidCheck(_):
        field_validation = False not in [
            ...
            and not (
                hasattr(field, "validation")
                and field.validation
                and field.validation(field.value)
            )
            for field in fields
        ]
        ...
        if date and date_options:
            ...
            if "min_age" in date_options:
                date_validation = dateCheck(
                    date.value,
                    allow_yrs=range(
                        current_year - 100, current_year - date_options["min_age"]
                    ),
                )
                ...
                date_validation = dateCheck(
                    date.value,
                    allow_yrs=range(
                        current_year, current_year + date_options["max_advance_yrs"] + 1
                    ),
                )
